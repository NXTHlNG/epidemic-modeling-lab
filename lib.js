function NewBlock(headers, idPrefix, rowNamesFunc, varNamesFunc) {
    ...
    fillRandom = function (chance, randomizeRow) {
        for (let i = 0; i < rows.length; i++) {
            ...
            if(randomizeRow) {
                ...
                for (let j = 0; j < inputs.length; j++) {
                    const v = randomChanced(chance, 1, 0);
...
/**
 * 
 * @param {number} chance шанс выпадение 0
 * @param {number} upper 
 * @param {number} lower
 * @returns random value between 0 and 1
 */
function randomChanced(chance, upper, lower) {

    if (Math.random() < chance) {
       return 0;
    }
    const value = lower + Math.random() * (upper - lower + Number.EPSILON); 
...
async function  postData(route, body, succsessCallback, errorCallback, callback) {
    await fetch(new Request(route, {
        method: "POST",
        body: body,
        headers: {
