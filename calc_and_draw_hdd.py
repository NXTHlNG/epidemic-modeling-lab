...
class CalculationDrawServiceHdd: 
    ...
    def calculate(self, coefs: dict) -> dict:
        ...
        # if np.any(Y < coefs["start"]):  
        #         raise ValueError("Some elements of Y are below the corresponding elements of start")
        return X, Y
    ...
    def save_plot(self, X, Y):
        ...
        for i in range(0, 13):
            line = plt.plot(X, Y[:, i], label=Constants.VARIABLES_DESCRIPTION[str(i + 1)]['variable_name'], color=colors[i])
            ...
            plt.annotate(Constants.VARIABLES_DESCRIPTION[str(i + 1)]['variable_title'].split()[0],
                         xy=(line_x[-1], line_y[-1]), xytext=(line_x[-1], line_y[-1]),
